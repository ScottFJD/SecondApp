...
public class InclusiveOrderActivity extends BaseActivity {
    ...
    @BindView(R.id.tv_inclusive_address)
    TextView tvInclusiveAddress;
    @BindView(R.id.tv_imm_pay)
    TextView tvImmPay;


    private String payMentType = "0";
    private CreateInclusiveModel orderModel;
    ...
    public static void start(Context context, CreateInclusiveModel model) {
    ...
    private void initView() {
        ...
        orderModel = (CreateInclusiveModel) bundle.getSerializable("orderDetail");
//        orderModel = (CreateInclusiveModel) getIntent().getSerializableExtra("orderDetail");
        tvLessonName.setText("土豪包场");
        tvOrderNum.setText(orderModel.getData().getPrivateUseOrderCode());
        tvOrderTime.setText(orderModel.getData().getDateStart());
        tvLessonAddress.setText(orderModel.getData().getDate()+" "+orderModel.getData().getTime());
        if (orderModel.getData().getSelectEmployee().equals("1")) {
            tvOrderKind.setText("是");
        } else {
            tvOrderKind.setText("否");
        }
        tvInclusiveAddress.setText(orderModel.getData().getFieldAddress());
        tvOrderPrice.setText(getResources().getString(R.string.RMB)+String.valueOf(orderModel.getData().getPrice()));
    }
    ...
    void setTvImmPay(View view) {
        if (!payMentType.equals("0")) {
            ...
            getPayment(decimalFormat.format(orderModel.getData().getPrice()),orderModel.getData().getPrivateUseOrderCode(),payMentType);
        } else {
...
//                            final String orderInfo = orderParam + "&" + sign;
                    ...
                    Runnable payRunnable = new Runnable() {
                        ...
                        @Override
                        public void run() {
                            PayTask alipay = new PayTask(InclusiveOrderActivity.this);
